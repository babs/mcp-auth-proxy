...
- **Business metrics**: alongside the Prometheus Go runtime counters, the `metrics` package emits `mcp_auth_tokens_issued_total{grant_type}`, `mcp_auth_access_denied_total{reason}`, `mcp_auth_replay_detected_total{kind}`, `mcp_auth_rate_limited_total{endpoint}`, `mcp_auth_clients_registered_total`, `mcp_auth_groups_claim_shape_mismatch_total` (id_token `groups` claim failed to decode as `[]string` — the user is admitted with empty groups, so this is NOT a denial; tracked separately so an IdP schema migration is visible before it cascades into a real `group` denial spike), and `mcp_auth_token_seals_total{purpose}` (successful AES-GCM seal operations per purpose; aggregate across replicas to track cumulative seals per signing key. The in-process L6 warning at 2^28 seals only fires per replica, so a frequently-rolled deployment never reaches it even when fleet-wide cumulative seals do — alert on `sum(increase(mcp_auth_token_seals_total[7d])) > 2**28` and rotate via the key-rotation runbook)
