...
- **Request headers are capped at 16 KB on both listeners** (`MaxHeaderBytes`), down from net/http's 1 MB default. Over that, net/http answers `431 Request Header Fields Too Large` before any middleware, so it appears in no access log and no metric. The `groups` claim inside an access token is capped at mint time (`GROUPS_CLAIM_MAX_BYTES`, default 8 KB, range `[1 KB, 10 KB]`) to keep the proxy's own bearer token inside that budget — watch `mcp_auth_groups_claim_truncated_total`.
...
| `GROUPS_CLAIM_MAX_BYTES` | Byte budget for the `groups` claim sealed into an access token; excess dropped at mint time with a `groups_claim_truncated` WARN naming the subject. Range `[1024, 10240]` — the ceiling is measured, since the seal expands the claim ~1.37x and 12 KB of groups no longer fits the 16 KB header block | `8192` |
...
- **Business metrics** (under `/metrics` on `METRICS_ADDR`):
  ...
  - `mcp_auth_groups_claim_truncated_total` — access tokens minted with a truncated `groups` claim (`GROUPS_CLAIM_MAX_BYTES`, default 8 KB). Authorization-visible: the claim is forwarded upstream as `X-User-Groups`, so a truncated user may be refused by the MCP server despite passing `ALLOWED_GROUPS` here
