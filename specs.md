...
- `TOKEN_SIGNING_SECRET` matching an obvious-weakness pattern (all-same byte, or short repeating period — e.g. `aaaa…`, `abcabc…`, `0123456789abcdef0123456789abcdef`) when `PROD_MODE=true`. Same gate applies to every entry in `TOKEN_SIGNING_SECRETS_PREVIOUS` so a rolling rotation cannot regress. Under `PROD_MODE=false` the same secret triggers a non-fatal weakness warning instead. Real random output of any encoding (raw, hex, base64) is non-periodic and passes; canonical generator is `manifests/scripts/generate-signing-secret.sh`.
...
- `token_signing_secret_weak` fires when the secret matches an obvious-weakness pattern (all-same byte, or repeating period < length) — signals a human-typed / patterned secret whose effective entropy is well below its length.
