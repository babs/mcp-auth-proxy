...
| 12 | D | **Sealed-input parse exhaustion** — oversized sealed blob slows `token.Open()` under load | Per-input length cap on every sealed-token open call; 1 MB body cap via `MaxBytesReader` on `/token`, `/consent`, `/register` | `token/token.go`, every handler entry point | `token/fuzz_test.go` (`FuzzOpenJSON`, `FuzzValidate`), `token/token_test.go` | — |
